...
namespace AssetTool
{
    public class AssetDependenciesTreeModel : AssetTreeModel
    {
        public override void SetDataPaths(string refPathStr, string pathStr, string commonPathStr)
        {
            ...
            if (AssetToolHandler.onDependenciesLoadDataMore != null)
            {
                AssetToolHandler.onDependenciesLoadDataMore(pathStr, resInfos, this);
            }
        ...
        {
            string path = AssetToolUtility.GetSaveFilePath(typeof(AssetDependenciesWindow).Name);
            ...
            AssetToolUtility.SaveFileText(path, sb.ToString());
            GUIUtility.ExitGUI();
        }
