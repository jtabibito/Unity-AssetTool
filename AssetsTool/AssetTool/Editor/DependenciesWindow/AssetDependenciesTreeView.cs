...
namespace AssetTool
{
    public class AssetDependenciesTreeView : AssetTreeView
    {
        ...
        public AssetDependenciesTreeView(TreeViewState state, MultiColumnHeader multiColumnHeader, AssetTreeModel model) : base(state, multiColumnHeader, model)
        {
            ...
            AssetToolHandler.onDependenciesFindItem += OnDependenciesFindItem;
        }
        ...
        protected override void ContextClickedItem(int id)
        {
            ...
            if (!IsSelectionMulti())
            {
                menu.AddItem(AssetToolStyle.Get().locationContext, false, OnContextSetActiveItem, id);
                menu.AddItem(AssetToolStyle.Get().explorerContext, false, OnContextExplorerActiveItem, item);
            }
            ...
            if (!IsSelectionContainsReverseItem())
            {
                ...
                menu.AddSeparator(String.Empty);
                menu.AddItem(AssetToolStyle.Get().dependenciesDelete, false, OnContextDeleteThisItem);

                if (AssetToolHandler.onDependenciesContextDraw != null)
                {
                    AssetToolHandler.onDependenciesContextDraw(menu);
                }
            }
