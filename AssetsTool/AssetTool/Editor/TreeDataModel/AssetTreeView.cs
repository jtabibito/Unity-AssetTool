...
namespace AssetTool
{
    public class AssetTreeView : TreeView
    {
        ...
        public AssetTreeView(TreeViewState state, MultiColumnHeader multiColumnHeader, AssetTreeModel model) : base(state, multiColumnHeader)
        {
            ...
            AssetToolWatcher.onImportedAssets += OnWatcherImportedAssets;
            AssetToolWatcher.onDeletedAssets += OnWatcherDeletedAssets;
            AssetToolWatcher.onMovedAssets += OnWatcherMovedAssets;
        }
        ...
        private void BuildDataDir(AssetTreeModel.AssetInfo dirInfo, TreeViewItem parent)
        {
            ...
            if (dirInfo.isFolder)
            {
                dirItem.icon = AssetToolStyle.Get().folderIcon;
            }
            ...
            string displayName, string fileRelativePath, bool deleted, bool added, bool contentIndent = true, bool foldoutIndent = false)
        {
            ...
            if (deleted || added)
            {
                ...
                position.height = 9f;
                position.width = 40f;
                GUI.DrawTexture(position, added ? AssetToolStyle.Get().iconNew.image:
                    AssetToolStyle.Get().iconDelete.image, ScaleMode.ScaleToFit);
            }
        }
        ...
        protected void AddContextMoveComm(GenericMenu menu)
        {
            if (m_Model.commonDirs != null)
            {
                ...
                {
                    menu.AddItem(new GUIContent(AssetToolStyle.Get().duplicateContextMoveComm + dir.displayName), false, OnContextMoveItem, dir.fileRelativePath);
                }
            }
        }
        ...
        protected virtual bool OnWatcherImportedAssetsEvent(string[] importedAssets, bool resortItem)
        {
            ...
            foreach (var importedAsset in importedAssets)
            {
                ...
                else
                {
                    ...
                    {
                        item2.icon = AssetToolStyle.Get().folderIcon;
                    }
                    assetInfo2.added = true;
                    item.AddChild(item2);
                }
