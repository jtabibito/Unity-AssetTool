...
namespace AssetTool
{
    public class AssetTreeModel
    {
        ...
        public virtual void SetDataPaths(string refPathStr, string pathStr, string commonPathStr)
        {
            ...
            ResetAutoId();
            assetPaths = pathStr;
            refPaths = AssetToolUtility.PathStrToArray(refPathStr);
            resPaths = AssetToolUtility.PathStrToArray(pathStr);
            commonPaths = AssetToolUtility.PathStrToArray(commonPathStr);
        ...
        public virtual void SetObject(UnityEngine.Object _object)
        {
            assetPaths = AssetDatabase.GetAssetPath(_object);
            data = new AssetInfo(_object.GetHashCode(), assetPaths, _object.name);
            data.bindObj = _object;
        }
        ...
        protected List<string> GetRefFileList()
        {
            ...
            foreach (var refPath in refPaths)
            {
                ...
                EditorUtility.DisplayProgressBar(AssetToolStyle.Get().progressTitle, String.Empty, 0f);
                var allFiles = Directory.GetFiles(refPath, "*", SearchOption.AllDirectories);

                for (var i = 0; i < allFiles.Length; i++)
                {
                    var file = allFiles[i];
                    if (!AssetToolUtility.IsPlainTextExt(file))
                    {
                        continue;
                    }
        ...
        protected List<string> GetResFileList()
        {
            ...
            foreach (var resPath in resPaths)
            {
                ...
                {
                    var file = allFiles[i];
                    if (AssetToolUtility.IsMetaExt(file))
                    {
                        continue;
                    }
        ...
        protected void ThreadDoFilesTextSearchReplace(List<string> fileList, List<string> patterns, string replaceStr, bool[][] searchRetList)
        {
            ...
            int numFinished = 0;
            AssetToolUtility.DisplayThreadProgressBar(numFiles, numFinished);
