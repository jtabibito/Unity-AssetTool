...
namespace AssetTool
{
    public class AssetBaseWindow : EditorWindow
    {
        ...
        public static void CheckObject<T>(UnityEngine.Object _object) where T : AssetBaseWindow
        {
            var window = GetWindow<T>();
            window.Focus();
            window.SetCheckObject(_object);
        }
        ...
        {
            if (GUILayout.Button(AssetToolStyle.Get().expandAll, EditorStyles.toolbarButton, GUILayout.Width(50f)))
            {
                m_AssetTreeView.ExpandAll();
            }
            if (GUILayout.Button(AssetToolStyle.Get().collapseAll, EditorStyles.toolbarButton, GUILayout.Width(50f)))
            {
        ...
        private void SetCheckObject(UnityEngine.Object _object)
        {
            m_CallbackAfterFrame = () =>
            {
                m_AssetTreeModel.SetObject(_object);
                if (m_AssetTreeModel.HasData())
                {
                    m_AssetTreeView.Reload();
                    m_AssetTreeView.ExpandAllExceptLast();
