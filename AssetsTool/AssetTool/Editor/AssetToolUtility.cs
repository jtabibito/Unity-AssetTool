...
namespace AssetTool
{
    public static class AssetToolUtility
    {
        ...
        public static void SaveFileText(string path, string text)
        {
            ...
            {
                EditorUtility.DisplayDialog(AssetToolStyle.Get().errorTitle, e.Message, AssetToolStyle.Get().sureStr);
                throw;
            }
        }
        ...
        {
            string msg = String.Format(@"{0} ({1}/{2})", AssetToolStyle.Get().progressTitle,
                (filesFinished + 1).ToString(), totalFiles.ToString());
            EditorUtility.DisplayProgressBar(AssetToolStyle.Get().progressTitle, msg, (filesFinished + 1) * 1f / totalFiles);
        }
    }
}